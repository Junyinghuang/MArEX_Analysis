...
def single(energy, crs, mean_dL, sig_dL, en, L):
    tot = 0
    norm = 0
    tof = energy_to_tof(en / 1e6, L, 939.56542, 299792458)
    energy_mean = tof_to_energy(tof, L + mean_dL, 939.56542, 299792458) * 1e6
    energy_low = tof_to_energy(tof, L + mean_dL - sig_dL, 939.56542, 299792458) * 1e6
    energy_high = tof_to_energy(tof, L + mean_dL + sig_dL, 939.56542, 299792458) * 1e6
    ...
    for i in range(100):
        ...
        cur += diff
    return tot / norm
def smear(energy, crs, mean_dL, sig_dL, en_low, en_high, num, L):
    ret_energy = []
    ret_crs = []
    diff = (en_high - en_low) / num
    cur = en_low
    for i in range(num):
        ret_crs.append(single(energy, crs, mean_dL, sig_dL, cur, L))
